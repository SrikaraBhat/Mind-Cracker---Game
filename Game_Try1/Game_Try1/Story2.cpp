...
char *Dialogs2[10] = { "It's Heavy", "I am loosing grip on it","Oh No it fell down","Its broken","Can you help me to fix this?", "Come on let's fix it" };
int Dcount2 = 6, D2 = 0;
int fall = 0;
float trans=0;
float flip = 0;
...
void keysp_Story2(int k, int x, int y)
{
	D2++;
	if (D2 >= Dcount2)
	{
...
{
	D2 = 0;
	glutSpecialFunc(keysp_Story2);
	glutIdleFunc(idle_story2);
}
void displayStory2()
{
...
//	int n1 = 0, i;
	...
	glColor3f(0, 0, 1);
	glRasterPos2f(0, 920);
	for (int i = 0; Dialogs2[D2][i] != '\0'; i++)
		glutBitmapCharacter(GLUT_BITMAP_8_BY_13, Dialogs2[D2][i]);
	glPushMatrix();
